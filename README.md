...
- 新規タスク追加時はタスク種別を選んでも日付指定を維持する
- 日付指定時は月間カレンダーを常時表示する
- 明日・今週中・来週では実際に保存される日付を表示し、カレンダーにも反映する
...
新規タスク追加時は `日付指定` が選択された状態で開きます。タスク種別を選んでも期限タイプは勝手に変えず、`日付指定` を維持します。期限日を未入力のまま保存しようとすると注意を表示します。期限なしで登録したい場合は、期限エリアで `期限なし` を明示的に選んでください。

`日付指定` では、アプリ内に月間カレンダーを常時表示します。日付セルをタップして期限日を選択し、選択中の日付と今日が分かるように表示します。ブラウザ標準の日付ピッカーをJavaScriptで無理に開かず、スマホとPCで見え方が安定するインラインカレンダーにしています。

`今日`、`明日`、`今週中`、`来週` を選んだ場合は、実際に保存される期限日を期限エリアに表示し、カレンダー上でも該当日を選択表示します。タスク種別の `default_due_type` は種別マスターの設定として保持しますが、新規追加時にタスク種別を選んだだけでは自動適用しません。
